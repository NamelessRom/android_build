...
class EdifyGenerator(object):
  ...
  def AssertDevice(self, device):
    ...
    cmd = ('assert(' +
           ' || \0'.join(['getprop("ro.product.device") == "%s" ||\0getprop("ro.build.product") == "%s"\0getprop("ro.product.device") == "%s" ||\0getprop("ro.build.product") == "%s"'
                          % (i, i, i.upper(), i.upper()) for i in device.split(",")]) +
