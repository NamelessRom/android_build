...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^nameless_") ; then
       NAMELESS_BUILD=$(echo -n $1 | sed -e 's/^nameless_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $NAMELESS_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       NAMELESS_BUILD=
    fi
    export NAMELESS_BUILD
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${NAMELESS_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    target=$1
    local variant=$2
    NAMELESS_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/nameless/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the nameless model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the nameless github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var NAMELESS_VERSION)
        ZIPFILE=nameless-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $NAMELESS_BUILD, run away!"
    fi
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $NAMELESS_FIXUP_COMMON_OUT ]; then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/nameless/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
